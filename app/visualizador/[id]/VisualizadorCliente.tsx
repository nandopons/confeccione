        ...
        {linhas.map((l, i) => {
          ...
          return (
            <div key={i} data-card-idx={i} className={"bg-white border rounded-2xl overflow-hidden scroll-mt-16 transition-all duration-500 " + (clonadoIdx === i ? "border-[#1D9E75] ring-2 ring-[#1D9E75] shadow-xl shadow-[#1D9E75]/20" : "border-gray-200 ring-1 ring-gray-900/5 shadow-md")}>
              ...
              <div role="button" onClick={(e) => { setAbertoIdx(i); const el = e.currentTarget; if (typeof window !== "undefined") setTimeout(() => el.scrollIntoView({ behavior: "smooth", block: "start" }), 80); }} className="scroll-mt-16 flex items-center justify-between gap-2 px-4 py-2.5 bg-[#0F6E56] text-white cursor-pointer select-none">
                ...
                <span className="inline-flex items-center gap-2 max-w-[60%]">
                  ...
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true" className={"shrink-0 transition-transform " + (i === abertoIdx ? "rotate-180" : "")}><path d="m6 9 6 6 6-6" /></svg>
                </span>
              </div>
              {/* CORPO COLAPSÁVEL — sanfona em todas as telas: só o card aberto aparece */}
              <div className={i === abertoIdx ? "block" : "hidden"}>
