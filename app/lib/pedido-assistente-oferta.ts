...
export type PedidoPago = {
  ...
  // Estes cinco existem pro match da tela ordenar os fornecedores. `linhas` é de
  // onde a peça é derivada; `peca`/`pecas` são só o piso (declaração da criação).
  cidade: string | null
  uf: string | null
 ...
 */
// A regra de match mora em ./match-fornecedor (módulo puro, sem supabase), pra
// poder ser importada por client component. Reexportado aqui porque metade do
...
export async function listarPedidosPagos(): Promise<{
  ...
  const { data: pedidosRaw } = await supabaseAdmin
    ...
    .select('id, codigo, criado_em, atualizado_em, nome, cep, valor_centavos, pagamento_status, confirmado_em, orcamento_status, status, telefone, email, prazo_dias, cidade, uf, categoria, peca, pecas, linhas')
...
export async function ofertarPedido(
  ...
  for (const fid of fornecedorIds) {
    ...
    if (forn.whatsapp) {
      ...
      const enviado = enviadoOficial || (await enviarTextoSimples(forn.whatsapp, mensagem))
