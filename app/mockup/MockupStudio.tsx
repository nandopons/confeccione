...
const SAUDACAO =
  "Oi! Escolhe a peça ali em cima, anexa sua logo e me diz como quer — ex.: “logo no peito esquerdo, de preto, menor”. Eu vou ajustando o mockup pra você. 😊";
...
export default function MockupStudio() {
  ...
  // troca de peça -> vista padrão + zona padrão
  function selecionarPeca(id: string) {
    const m = getMockup(id);
    if (!m) return;
    setPecaId(id);
    const vp = m.vistaPadrao;
    setVista(vp);
    setPosicao(m.vistas[vp]?.zonaPadrao ?? "centro");
    setCorLogo(m.corLogoSugerida === "branco" ? "branco" : "original");
    setOff({ x: 0, y: 0 });
  }

  // troca de vista -> zona padrão da vista
  function trocarVista(v: Vista) {
    if (!peca?.vistas[v]) return;
    setVista(v);
    setPosicao(peca.vistas[v]!.zonaPadrao);
    setOff({ x: 0, y: 0 });
  }

  // carrega imagem do mockup ao mudar peça/vista
  useEffect(() => {
    if (!view) return;
    const img = new Image();
  ...
  // processa a logo (knockout de fundo + recolor)
  useEffect(() => {
  ...
  useEffect(() => { render(); /* eslint-disable-next-line */ }, [tamanho, off, posicao, logoVersao, vista, pecaId]);
  ...
  function processarLogo(img: HTMLImageElement, knockout: boolean, cor: CorLogo): HTMLCanvasElement {
    ...
    if (knockout) {
      ...
      let r = 0, g = 0, b = 0;
      for (const [x, y] of pts) { const i = (y * w + x) * 4; r += a[i]; g += a[i + 1]; b += a[i + 2]; }
      r /= 4; g /= 4; b /= 4;
      const tol = 52;
      for (let i = 0; i < a.length; i += 4) {
        const dr = a[i] - r, dg = a[i + 1] - g, db = a[i + 2] - b;
        if (Math.sqrt(dr * dr + dg * dg + db * db) < tol) a[i + 3] = 0;
      }
  ...
  function logoRect() {
    ...
    if (!img || !logo || !view) return null;
    const zona = view.zonas[posicao] ?? Object.values(view.zonas)[0];
    if (!zona) return null;
    const W = img.naturalWidth, H = img.naturalHeight;
    const area = { x: zona.x * W, y: zona.y * H, w: zona.w * W, h: zona.h * H };
    const ratio = logo.height / logo.width;
    let lw = area.w * tamanho;
    let lh = lw * ratio;
    const maxH = area.h * 2.4;
    if (lh > maxH) { lh = maxH; lw = lh / ratio; }
    const cx = area.x + area.w / 2 + off.x;
    const cy = area.y + area.h / 2 + off.y;
    return { x: cx - lw / 2, y: cy - lh / 2, w: lw, h: lh };
  }

  function render() {
    const canvas = canvasRef.current, img = mockupImgRef.current;
    ...
    ctx.drawImage(img, 0, 0);
    const lr = logoRect();
    if (lr) { ctx.save(); ctx.globalAlpha = 0.97; ctx.drawImage(logoProcRef.current!, lr.x, lr.y, lr.w, lr.h); ctx.restore(); }
  }

  function evNat(e: React.PointerEvent) {
    const c = canvasRef.current!; const rect = c.getBoundingClientRect();
    return { x: (e.clientX - rect.left) * (c.width / rect.width), y: (e.clientY - rect.top) * (c.height / rect.height) };
  }
  function onDown(e: React.PointerEvent) {
    if (!logoProcRef.current) return;
    const p = evNat(e); dragRef.current = { ativo: true, px: p.x, py: p.y };
  ...
  function onUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => { setLogoSrc(typeof reader.result === "string" ? reader.result : null); setLogoNome(file.name); setOff({ x: 0, y: 0 }); };
    reader.readAsDataURL(file);
  }

  async function enviar() {
    const texto = input.trim(); if (!texto || enviando) return;
    setErro(null);
    const novo: Turno = { role: "user", display: texto, raw: texto };
    const base = [...turnos, novo]; setTurnos(base); setInput(""); setEnviando(true);
    const messages = base.filter((t) => !t.seed).map((t) => ({ role: t.role, content: t.raw || t.display }));
    const estado = { pecaId, vista, corLogo, removerFundo, posicao, tamanho, temLogo: !!logoSrc };
    try {
  ...
  function aplicarOps(ops: Record<string, unknown>) {
    if (typeof ops.pecaId === "string" && getMockup(ops.pecaId)) {
      const m = getMockup(ops.pecaId)!; setCategoriaId(m.categoria); selecionarPeca(ops.pecaId);
    }
    const p = getMockup((ops.pecaId as string) || pecaId) ?? peca;
    if (typeof ops.vista === "string" && p?.vistas[ops.vista as Vista]) trocarVista(ops.vista as Vista);
    if (typeof ops.posicao === "string") {
      const v = (ops.vista as Vista) || vista;
      const zonas = p?.vistas[v]?.zonas ?? {};
      if ((zonas as Record<string, unknown>)[ops.posicao as string]) { setPosicao(ops.posicao as ZonaNome); setOff({ x: 0, y: 0 }); }
    }
    ...
    if (typeof ops.tamanho === "number") setTamanho(Math.min(1.3, Math.max(0.3, ops.tamanho)));
  }
  ...
  function usarNoPedido() {
    try { const c = canvasRef.current; sessionStorage.setItem("mockup_pedido", JSON.stringify({ peca: peca?.nome, pecaId, vista, imagem: c ? c.toDataURL("image/jpeg", 0.8) : null })); } catch { /* */ }
  }
  ...
  return (
    <section className="bg-[#F7F8F9]">
      <div className="max-w-5xl mx-auto px-6 py-12 space-y-6">
        {/* 1. Biblioteca: categorias + carrossel */}
        <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5">
            ...
            {CATEGORIAS.map((c) => (
              <button key={c.id} type="button" onClick={() => setCategoriaId(c.id)}
                className={"whitespace-nowrap px-3 py-1.5 rounded-full text-xs font-medium border transition-colors " +
                  (c.id === categoriaId ? "bg-[#111] text-white border-[#111]" : "border-gray-200 text-gray-600 hover:bg-gray-50")}>
                {c.nome}
              </button>
            ))}
          </div>
          <div className="flex gap-3 overflow-x-auto pt-3 -mx-1 px-1">
            {pecasDaCategoria.length === 0 && (
              <p className="text-gray-400 text-sm py-6">Novos mockups dessa categoria em breve.</p>
            )}
            {pecasDaCategoria.map((m) => {
              const capa = m.vistas[m.vistaPadrao]?.arquivo ?? "";
              return (
                <button key={m.id} type="button" onClick={() => selecionarPeca(m.id)}
                  className={"flex-shrink-0 w-28 border-2 rounded-xl overflow-hidden text-left transition-all " +
                    (m.id === pecaId ? "border-[#1D9E75]" : "border-gray-200 hover:border-gray-300")}>
        ...
        <div className="grid gap-6 md:grid-cols-[1fr_1.1fr]">
          ...
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm flex flex-col overflow-hidden">
            <div className="px-5 py-4 border-b border-gray-100 flex items-center justify-between">
              ...
              <button type="button" onClick={() => fileRef.current?.click()} className="text-xs font-medium text-[#0F6E56] hover:underline">
                {logoNome ? "Trocar arte" : "Anexar logo"}
              </button>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5 flex flex-col">
                ...
                {(["frente", "costas", "lateral"] as Vista[]).filter((v) => peca?.vistas[v]).map((v) => (
                  <button key={v} type="button" onClick={() => trocarVista(v)}
                    className={"px-2.5 py-1 rounded-lg text-xs border capitalize transition-colors " +
                      (v === vista ? "border-[#1D9E75] bg-[#E1F5EE] text-[#0F6E56]" : "border-gray-200 text-gray-500 hover:bg-gray-50")}>{v}</button>
                ))}
              </div>
            </div>
            <div className="flex-1 rounded-xl bg-gray-50 border border-gray-100 flex items-center justify-center overflow-hidden">
              <canvas ref={canvasRef} onPointerDown={onDown} onPointerMove={onMove} onPointerUp={onUp} onPointerLeave={onUp}
                className="max-w-full max-h-[460px] object-contain touch-none cursor-move" />
            </div>

            {logoSrc && (
              <div className="mt-3">
                <p className="text-xs text-gray-500 mb-1">Tamanho da arte</p>
                <input type="range" min={0.3} max={1.3} step={0.02} value={tamanho} onChange={(e) => setTamanho(parseFloat(e.target.value))} className="w-full accent-[#1D9E75]" />
                <p className="text-[11px] text-gray-400 mt-1">Arraste a arte na prévia pra posicionar. O resto, peça no chat.</p>
              </div>
            )}
            ...
            <div className="mt-4 flex flex-col sm:flex-row gap-2">
              ...
              <Link href="/#pedido" onClick={usarNoPedido}
                className={"flex-1 text-center px-4 py-2.5 rounded-xl text-sm font-medium transition-colors " +
                  (logoSrc ? "bg-[#1D9E75] hover:bg-[#0F6E56] text-white" : "bg-gray-100 text-gray-400 pointer-events-none")}>Usar no meu pedido →</Link>
            </div>
          </div>
        </div>
      </div>
